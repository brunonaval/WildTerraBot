...
namespace WildTerraBot
{
    public class UDPRunner : MonoBehaviour
    {
            ...
            void Start()
            {
                // CORREÇÃO: Inicia o Dump aqui para não crashar o PlayerHooks
                ...
                // Reflection Init (Resumido para espaço, mas funcional)
            ...
            // CORREÇÃO DO CRASH: O Dump agora roda aqui, seguro.
            IEnumerator AutoDumpDelay()
            {
                yield return new WaitForSeconds(5.0f);
                if (WTSocketBot.Instance != null && !WTSocketBot.HasDumped)
                {
                    WTSocketBot.Instance.DumpDeepSecrets("AUTO-START");
            ...
            private void ListenLoop(UdpClient listener)
            {
                ...
                while (running)
                {
                    try
                    {
                        ...
                        if (p[0] == "DUMP") mainThreadActions.Enqueue(() => WTSocketBot.Instance.DumpDeepSecrets("UDP-TRIGGER"));
            ...
            void Update()
            {
                ...
                if (_lastKnownHp != -1 && currentHp < _lastKnownHp) { _lastDamageTime = Time.time; }
                ...
                bool tomandoDano = (Time.time - _lastDamageTime < 3.0f);
                ...
                bool temAlvo = (_combatTarget != null);

                if (emCombateReal || temAlvo)
                {
